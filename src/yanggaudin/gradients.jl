function gradient(H::LocalHamiltonian, Ψρs::YangGaudinCMPSData, HL=nothing, HR=nothing;
                  kwargs...)
    Ψ, ρL, ρR = Ψρs
    if isnothing(HL)
    ...
    # gradR = ∑(coeff * localgradientR)  +  2*HL*R*ρR + 2*ρL*R*HR
    gradRs = zero.(Rs)
    for (coeff, op) in zip(coefficients(H.h), operators(H.h))
        if coeff isa Number
    ...
    mul!.(gradRs, (HL,), Rs .* (ρR,), 2, 1)
    mul!.(gradRs, (ρL,), Rs .* (HR,), 2, 1)
