defaulteigalg(Ψ::YangGaudinCMPS) = Arnoldi(; krylovdim=min(256, virtualdim(Ψ)^2))
defaultlinalg(Ψ::YangGaudinCMPS) = GMRES(; krylovdim=min(256, virtualdim(Ψ)^2))

# CMPS environements
function leftenv(Ψ::YangGaudinCMPS, ρ₀=one(Ψ.Q);
...
function leftenv!(Ψ::YangGaudinCMPS, ρ₀=one(Ψ.Q); kwargs...)
...
function rightenv(Ψ::YangGaudinCMPS, ρ₀=one(Ψ.Q);
                  eigalg=defaulteigalg(Ψ),
                  linalg=nothing, # ignored
                  kwargs...)
    eigsort = EigSorter(x -> (abs(div(imag(x), pi / period(Ψ))), -real(x)))
...
function rightenv!(Ψ::YangGaudinCMPS, ρ₀=one(Ψ.Q); kwargs...)
...
function environments!(Ψ::YangGaudinCMPS, ρL₀=one(Ψ.Q), ρR₀=one(Ψ.Q); kwargs...)
...
const YangGaudinCMPSData = Tuple{YangGaudinCMPS,PeriodicMatrixFunction,
                                 PeriodicMatrixFunction}

function leftenv!(H::LocalHamiltonian, Ψ::YangGaudinCMPS, HL₀=zero(Ψ.Q); kwargs...)
    domain(H) == domain(Ψ) || throw(DomainMismatch())
    ρL, ρR, infoL, infoR = environments!(Ψ; kwargs...)
    return leftenv(H, (Ψ, ρL, ρR), HL₀; kwargs...)
end

function rightenv!(H::LocalHamiltonian, Ψ::YangGaudinCMPS, HR₀=zero(Ψ.Q); kwargs...)
    domain(H) == domain(Ψ) || throw(DomainMismatch())
    ρL, ρR, infoL, infoR = environments!(Ψ; kwargs...)
...
function leftenv(H::LocalHamiltonian, Ψρs::YangGaudinCMPSData, HL₀=nothing;
...
function rightenv(H::LocalHamiltonian, Ψρs::YangGaudinCMPSData, HR₀=nothing;
                  eigalg=nothing, # ignored
                  linalg=defaultlinalg(Ψρs[1]),
                  kwargs...)
    Ψ, ρL, ρR = Ψρs
...
function environments!(H::LocalHamiltonian, Ψ::YangGaudinCMPS; kwargs...)
...
function environments(H::LocalHamiltonian, Ψρs::YangGaudinCMPSData,
